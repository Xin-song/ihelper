...
> 🚧 **Phase 1（菜谱 MVP）基本做完**，Phase 2 的库存、待办与日程也提前做了不止最小版。
> 菜谱模块可用：CRUD、分量缩放、图片上传、菜谱广场、交作业、可打印菜谱图、**搜索与筛选**
> （关键词搜标题/简介/步骤、按分类/标签/难度/总耗时筛选、**食材反查**「有这些食材能做什么」、按最近添加/最近制作/评分/耗时排序）。
> 登录/注册已接入（用户名 + 密码），库存管理（分类、数量、安全库存、自动生成采购清单）已接入。
> 待办与日程已接入：日历（月/周/日，月视图按当周最忙的一天动态撑高）、看板（进行中 + 未来三天按优先级）、
> 美国联邦假日标注，以及**待办事项主题看板**——先建「主题」（文件夹），主题下的待办可勾选划线完成、
> 归档/取消归档、写详情备注；「今日日程管理」视图里待办可以拖到时间轴上（或点「+」直接加入），
> 拖色块能整体挪动、拖上下边框能调整起止时间。日历/看板/待办/今日日程管理/Archived 五个 tab 是平级的，
> 共用同一份数据，任意一个 tab 改了状态，其余 tab 立刻同步，不用手动刷新。
> 顶部导航是 App 切换器（iHelper logo 悬浮菜单 → Cook / Calendar），菜谱与日程是并列应用。
> 数据仍挂在默认空间（Phase 3 前恒为单空间），购物清单（多菜谱合并版）待做。
...
- [x] 待办事项主题看板（主题分组、归档、详情备注、今日日程可拖拽/点击排期并能拖边框调整时长），
      日历/看板/待办/今日日程管理/Archived 五个 tab 平级、共用数据、状态互相同步，Phase 2 提前做
- [x] 菜谱搜索与筛选（关键词全文 + 分类/标签/难度/耗时筛选 + 食材反查 + 排序）
